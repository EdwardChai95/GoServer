...
func VerifyJWTGetClubUser(httpRequest *http.Request) (map[string]string, bool) {
	if !IsVerifiedIP(httpRequest) {
		return nil, false
	}
	...
	if isValid {
		...
		if userinfo, ok := claims[USERINFOKEY]; ok {
			iter := reflect.ValueOf(userinfo).MapRange()
			for iter.Next() {
				k := fmt.Sprintf("%v", iter.Key())
				if float64_v, ok := iter.Value().Interface().(float64); ok && (k == "uid" || k == "Uid") {
					s := strconv.FormatFloat(float64_v, 'f', -1, 64)
					s = strings.Trim(s, ".")
					payload[k] = s
				} else if int64_v, ok := iter.Value().Interface().(int64); ok {
					payload[k] = Int64ToString(int64_v)
				} else {
					// reqFormat[k] = fmt.Sprintf("%v", v)
					payload[k] = fmt.Sprintf("%v", iter.Value())
				}
				// fmt.Printf("Key: %v, value: %v\n", iter.Key(), iter.Value())
			}
			return payload, true
		} else {
			payload["uid"] = fmt.Sprintf("%v", claims[TOKENUIDKEY])
		}
		return payload, true
	}
	return nil, false
...
func VerifyAdminJWT(httpRequest *http.Request) (string, string, bool, bool) { // for request from admin dashboard
	if !IsVerifiedIP(httpRequest) { // verify ip first new requirement added in may 2021
		return define.IPBLOCKEDMESSAGE, "", false, false
	}
	tokenString := httpRequest.Header.Get("Authorization")
	if tokenString == "" {
		return "", "", false, false
	...
	if isValid {
		isAdmin := false
		val := ""
		if adminval, ok := claims[TOKENADMINUIDKEY]; ok {
			val = fmt.Sprintf("%v", adminval)
			isAdmin = ok
		}
		return fmt.Sprintf("%v", claims[TOKENUIDKEY]), val, true, isAdmin
	}
	return "", "", false, false
}
...
func VerifyJWT(httpRequest *http.Request) (string, bool) {
	if !IsVerifiedIP(httpRequest) { // verify ip first new requirement added in may 2021
		return define.IPBLOCKEDMESSAGE, false
	}
	tokenString := httpRequest.Header.Get("Authorization")
	if tokenString == "" {
		return "", false
...
func IsAdminBool(user *model.User) bool {
	isAdmin := false
