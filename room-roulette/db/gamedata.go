...
func GetDailyCollectionByNameAndRoom(game_name, game_room string) map[string]string {
	game_collections, err := db.QueryString("select * from `daily_collection` " +
		"where `game_name` = '" + game_name + "' AND `game_room` = '" + game_room + "' LIMIT 1")
...
func DailyCollection(game_name, game_room string, game_coin_collection, game_coin_betting int64) {
	game_coin_percentage := "0.0"
	if game_coin_collection > 0 && game_coin_betting > 0 {
		game_coin_percentage = fmt.Sprintf("%.2f", float64(game_coin_collection)/float64(game_coin_betting)*100.0)
	}

	if gameCollection := GetDailyCollectionByNameAndRoom(game_name, game_room); gameCollection != nil {
		t := time.Now()
		loc, _ := time.LoadLocation("Asia/Ho_Chi_Minh")
		t_test := time.Date(t.Year(), t.Month(), t.Day(), 0, 0, 0, 0, t.Location())
		t_test.In(loc)
		createdFormat := "2006-01-02 15:04"
		t1 := GetCurrentShanghaiTime().Format(createdFormat)
		t2 := t_test.Format(createdFormat)
		fmt.Println("t1:", t1)
		fmt.Println("t2:", t2)
		fmt.Println(gameCollection["game_coin_betting"])
		if t1 == t2 {
			_, err := db.Exec("update `daily_collection` set `game_coin_collection` = ?, "+
				"`game_coin_betting` = ?, `game_coin_percentage` = ? where `game_collection_id` = ?",
				0, 0, 0, gameCollection["game_collection_id"])
			if err != nil {
				logger.Printf("daily_collection update err: %v", err)
			}
			return
		} else {
			betting := StringToInt64(gameCollection["game_coin_betting"]) + game_coin_betting
			collection := StringToInt64(gameCollection["game_coin_collection"]) + game_coin_collection
			percentage := StringToFloat64(gameCollection["game_coin_percentage"]) + StringToFloat64(game_coin_percentage)
			_, err := db.Exec("update `daily_collection` set `game_coin_collection` = ?, "+
				"`game_coin_betting` = ?, `game_coin_percentage` = ? where `game_collection_id` = ?",
				collection, betting, percentage, gameCollection["game_collection_id"])
			if err != nil {
				logger.Printf("daily_collection update err: %v", err)
			}
			return
	...
	_, err := db.Exec("INSERT INTO `daily_collection`(`game_name`, `game_room`, `game_coin_collection`, `game_coin_betting`,`game_coin_percentage`)"+
		" VALUES (?, ?, ?, ?, ?)",
		game_name, game_room, game_coin_collection, game_coin_betting, game_coin_percentage)
	if err != nil {
		logger.Printf("insert update err: %v", err)
...
func RecordWinningItem(winning_animal string, winning_zone string, winning_text string, level string) { // new level
	currentTime := getCurrentShanghaiTimeString()
	affected, err := db.Exec("INSERT INTO `roulette_winning_items` "+
		"(`winning_animal`, `winning_zone`, `winning_text`, `level`, `create_at`) "+
		"VALUES (?, ?, ?, ?, ?)",
		winning_animal, winning_zone, winning_text, level, currentTime)
	if err != nil {
		log.Println(err)
	}
...
func GetClubByUid(uid int64) map[string]string {
	users, err := db.QueryString("select * from club_user where uid = '" +
		strconv.Itoa(int(uid)) + "' LIMIT 1")
	if err != nil {
		log.Println(err)
	}
